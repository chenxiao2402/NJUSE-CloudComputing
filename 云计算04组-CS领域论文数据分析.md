...
1. 数据量过大导致运算时间过长。
   ...
   - 这是最大的困难，迫使我们多次修改实现方式，以及做出妥协。一开始我们希望

   - 解决方案：

2. 运算结果数据过大以至于无法预先算好并存入MySQL数据库。

   - 对于作者间接合作关系搜索，我们最初的想法是像其他图的计算一样，预先算好结果存入MySQL数据库，让后端直接从中读取结果。但初步实现并测试后发现，对于一篇有n个作者的论文，我们就有 $xz$ 
   - 解决方案：

3. 操作`DataFrame`和`RDD`时不容易弄清楚返回结果的结构。
